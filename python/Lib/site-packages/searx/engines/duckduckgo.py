...
_CACHE = None  # RSS-REEL PATCH
"""Persistent (SQLite) key/value cache that deletes its values after ``expire``
seconds."""


def _cache() -> EngineCache:  # RSS-REEL PATCH
    """Return the cache, creating it on first use.

    RSS-REEL PATCH (2026-09-01). `CACHE` was ONLY a type annotation, and its value
    was created solely in this module's `init()`. When another engine uses this
    module - `duckduckgo news` from `settings.yml`, `duckduckgo_extra`,
    `duckduckgo_weather` - `init()` is not called from here, and the first access
    to `CACHE` in `get_vqd()` ended with:

        searx.engines.duckduckgo news : NameError: name 'CACHE' is not defined

    The engine then failed on EVERY query. Lazy creation removes the dependency
    on who called `init()` and who did not.

    ⚠ THE UNDERSCORE IN THE NAME MATTERS (fix of 2026-09-03). The first version
    of this patch wrote `CACHE = None` at module level - and then the engine loader
    (`searx/engines/__init__.py: is_missing_required_attributes`) started shouting

        ERROR searx.engines: Missing engine config attribute: "duckduckgo.CACHE"

    and `load_engine` returned `None`, i.e. the engine was NOT LOADED AT ALL. That
    test treats as required every module attribute that does not start with `_`
    and has the value `None`. Before, `CACHE` was a bare annotation, so it was not
    in `dir()`. Fixing the NameError at query time turned it into a refusal to
    load. `_CACHE` passes that test unnoticed.

    ⚠ The rename is safe: nothing reaches for `CACHE` from outside.
    `duckduckgo_extra.py` and `duckduckgo_weather.py` import only
    `fetch_traits`, `get_ddg_lang` and `get_vqd` from here.
    """
...
def init(_):  # pylint: disable=unused-argument
    _cache()  # RSS-REEL PATCH: the same path as lazy creation
...
def get_vqd(query: str, region: str, force_request: bool = False) -> str:
    ...
    """
    key = _cache().secret_hash(f"{query}//{region}")  # RSS-REEL PATCH
    value = _cache().get(key=key)  # RSS-REEL PATCH
    ...
    if value:
        _cache().set(key=key, value=value)  # RSS-REEL PATCH
    else:
...
def set_vqd(query: str, region: str, value: str):
    key = _cache().secret_hash(f"{query}//{region}")  # RSS-REEL PATCH
    _cache().set(key=key, value=value, expire=3600)  # RSS-REEL PATCH
